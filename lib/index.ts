...
// 读取设置 ========================
/* eslint-disable */
console.log('欢迎使用晓狩 Bot！')
console.log('=========================')
const configSelect = db.prepare('SELECT * FROM bot_config').all()
if (configSelect.length == 0) {
    ...
    for (const index in configSelect) {
        const key = (configSelect[index] as { [key: string]: any }).key
        const value = (configSelect[index] as { [key: string]: any }).value
        config[key] = value
    }
    console.log('> 如需重新配置，请删除数据库文件中的 bot_config 表')
